...
* 최대한 유연하게 만들어야 함(`캡슐화`)
	* 인터페이스(상위 형식) 사용
		* 이런 식으로 디자인하면 다른 형식의 객체에서도 `나는 행동`과 `꽥꽥거리는 행동` 재사용 가능
		* 그런 행동이 더 이상 Duck 클래스 안에 숨겨있지 않기 때문
	* 기존의 행동 클래스 변경 없어야 함
...
* 설계  
	<img src="./img/duck_hierarhcy_6.jpg" width="350" height="300"></br>
* 코드
	* X  
		```java
		Dog d = new Dog();
...
### 결론
위에서 했던 바와 같이 가장 중요한 점은 Duck에서 `나는 행동`과 `꽥꽥 소리를 내는 행동`을 Duck 클래스(또는 그 서브 클래스)에서 정의한 메소드를 써서 구현하지 않고 다른 클래스에 위임하는 것이었다.

그렇다면 이제는 분리된 Duck의 행동을 통합해야 한다. 코드로 살펴본다.

```java
public class Duck {
	FlyBehavior flyBehavior;	// 인터페이스 형식의 인스턴스 변수 선언(초기화 X)
...
#### 상속보다는 구현
상속과 구현을 표현하는 다른 표현이 있다.

* 상속: `"A는 B이다"`
* 구현: `"A에는 B가 있다"`
	* 각 오리에는 `FlyBehavior`와 `QuackBehavior`가 있으며 각각 나는 행동과 꽥꽥거리는 행동을 위임받음
	* 두 클래스를 이와 같이 합치는 것을 구성(`composition`)이라고 부름

위에서 봐왔듯, 구성을 이용하여 시스템을 만들면 유연성을 크게 향상시킬 수 있다. 따라서 아래와 같은 디자인 원칙이 있다.

> 상속보다는 구성을 활용한다.

이 패턴을 스트레티지 패턴(strategy pattern)이라고 한다.
...
* 불필요한 상속의 예: `java.util.Stack`와 `java.util.Properties`
* `asList` 관련 언급(`재질문`)
* 스프링 관련 개념: `AOP`와 `DI`
* 교재 추천: [스프링 입문을 위한 자바 객체 지향의 원리와 이해](https://m.blog.naver.com/PostView.nhn?blogId=hji7000&logNo=220843863268&proxyReferer=https%3A%2F%2Fwww.google.com%2F)

##### [목차로 이동](#목차)

## 참고
* [Strategy pattern - LichKing](https://multifrontgarden.tistory.com/256)
