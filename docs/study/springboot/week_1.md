...
- [x] Open API 호출 시 RestTemplate를 사용하며, 별도의 Bean 으로 정의한다.(@Configuration, @Bean 사용)
...
코드에 대해 받은 피드백은 아래와 같다.

* 블로그 정보 조회 시 null 조회 → 네이버 OPENAPI 스펙과 변수명 통일
* 배열 대신 LIST 사용 권장 → 이펙티브 자바 아이템 28
* URL 생성 Bean 선언 유연성 낮음 → 다른 API 조회 시 생각
* 필드 선언 시 private final 선언 필요

- - -

* 외부 설정을 통해 어플리케이션의 설정값 세팅
